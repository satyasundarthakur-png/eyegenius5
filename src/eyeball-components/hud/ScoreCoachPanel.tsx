  ...
  return (
    <div className="pointer-events-auto w-64 rounded-lg border border-blue-500/30 bg-gray-950/85 p-3 text-blue-100 backdrop-blur">
      <div className="mb-2 flex items-center justify-between border-b border-blue-500/20 pb-1">
        ...
        <span className="font-mono text-lg font-bold text-white">{Math.round(performance.overall)}</span>
      </div>
          ...
          {showHistory && (
            <ul className="mt-1 space-y-1">
              {previousNotes.map((note, i) => (
                <li key={i} className="rounded border border-blue-500/10 bg-black/15 px-1.5 py-1 text-[10px] text-blue-200/60">
                  {note.message}
                </li>
              ))}
            </ul>
