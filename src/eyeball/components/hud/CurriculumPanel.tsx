...
const STEP_ACTION: Record<string, string> = {
  incision: "Set tilt ≥ 15° (preset 2), drag down to >2.5 mm depth.",
  capsulorhexis: "Drag to 1.5–4.5 mm depth at low angle.",
  hydrodissection: "Wait for AC to pressurise (bar → green), then drag to >1 mm depth.",
  phacoemulsification: "Drag to 3–8 mm depth in the nuclear zone.",
  cortex_removal: "Let vacuum build (bar fills), maintain stable chamber.",
  iol_insertion: "Drag to 0.5–3.5 mm depth for controlled lens delivery.",
  wound_hydration: "Drag to 0.5–2.0 mm depth to hydrate and seal wound edges.",
};
    ...
    return (
      <div className="text-blue-100">
        ...
        <p className="mb-1 text-xs text-blue-200/70 leading-snug">
          7-step guided procedure: incision → capsulorhexis → hydrodissection → phaco → cortex
          removal → IOL → wound hydration.
        </p>
        <p className="mb-3 text-xs text-blue-300/50 leading-snug">
          First: press{" "}
          <kbd className="rounded border border-blue-500/30 bg-blue-500/15 px-1 font-mono text-[10px]">
            P
          </kbd>
    ...
    return (
      <div className="text-blue-100">
        ...
        {complications.length > 0 && (
          <div className="mb-3">
            <p className="mb-1 text-[10px] uppercase tracking-wide text-red-400">
  ...
  return (
    <div className="text-blue-100">
      ...
      <ol className="mb-3 space-y-0.5">
        {STEP_ORDER.map((step, i) => {
          ...
          return (
            ...
            >
              <span className="w-4 text-center font-mono text-[10px]">{isDone ? "✓" : i + 1}</span>
              {STEP_LABEL[step]}
            </li>
          );
        })}
      ...
      <div className="mb-2 rounded border border-blue-500/20 bg-blue-950/40 px-2 py-2 text-xs">
        ...
        {neededInstrument && (
          <div
            className={`mb-1.5 flex items-center justify-between rounded px-1.5 py-1 ${
              wrongInstrument
          ...
          >
            ...
            {wrongInstrument && (
              <button
                onClick={() => {
                  setCurrentInstrument(neededInstrument);
                }}
