    ...
    >
      <div className="flex h-full flex-col overflow-hidden rounded-xl border border-blue-500/20 bg-gray-950/92 shadow-2xl backdrop-blur-md">
        ...
        >
          ...
          <Section id="curriculum" icon={GraduationCap} label="Surgery Curriculum"
            accent="text-purple-400" open={openSection === 'curriculum'}
            onToggle={() => { toggle('curriculum'); }}>
            {curriculumPanel}
          </Section>

          <Section id="procedure" icon={Layers} label="Procedure"
            accent="text-blue-400" open={openSection === 'procedure'}
            onToggle={() => { toggle('procedure'); }}>
            {procedurePanel}
          </Section>

          <Section id="instrument" icon={Wrench} label="Instrument"
            accent="text-green-400" open={openSection === 'instrument'}
            onToggle={() => { toggle('instrument'); }}>
            {instrumentPanel}
          </Section>

          <Section id="mode" icon={Settings} label="Mode & Controls"
            accent="text-blue-400" open={openSection === 'mode'}
          ...
          <Section id="microscope" icon={MicroscopeIcon} label="Microscope"
            accent="text-cyan-400" open={openSection === 'microscope'}
            onToggle={() => { toggle('microscope'); }}>
            {microscopePanel}
          </Section>

          <Section id="telemetry" icon={Activity} label="Instrument Telemetry"
            accent="text-amber-400" open={openSection === 'telemetry'}
            onToggle={() => { toggle('telemetry'); }}>
            {kinematicsPanel}
          </Section>

          <Section id="score" icon={Award} label="Score & AI Coach"
            accent="text-amber-400" open={openSection === 'score'}
            onToggle={() => { toggle('score'); }}>
            {scorePanel}
          </Section>

          <Section id="minimap" icon={Map} label="Minimap"
            accent="text-blue-400" open={openSection === 'minimap'}
            onToggle={() => { toggle('minimap'); }}>
            {minimapPanel}
          </Section>

          <Section id="manual" icon={LifeBuoy} label="User Manual"
            accent="text-emerald-400" open={openSection === 'manual'}
...
const GLOSSARY = [
  ...
  { term: 'Red Reflex', def: 'Fundal orange-red glow through the dilated pupil under coaxial light.' },
  { term: 'Mydriasis', def: 'Pharmacological dilation of the pupil with drops (tropicamide / phenylephrine).' },
  { term: 'IOL', def: 'Intraocular lens inserted into the capsular bag after nucleus removal.' },
  { term: 'Hydrodissection', def: 'Injection of fluid under the capsule to free the cortex from the capsular bag.' },
  { term: 'Nuclear Sclerosis', def: 'Age-related hardening and yellowing of the lens nucleus (the cataract itself).' },
  { term: 'Capsular Bag', def: 'The thin membrane envelope that holds the lens and receives the IOL.' },
  ...
  { term: 'Wound Hydration', def: 'Injecting BSS into the corneal incision stroma to seal it watertight.' },
  { term: 'Entry Point', def: 'The incision site on the sclera/limbus through which instruments are inserted.' },
  { term: 'RCM', def: 'Remote Centre of Motion — the geometric pivot point at the incision site.' },
];

const STEPS = [
  { n: 1, name: 'Incision', inst: 'Keratome', tip: 'Insert 2.5–4 mm at a shallow angle into the limbal zone.' },
  { n: 2, name: 'Capsulorhexis', inst: 'Capsulorhexis Forceps', tip: 'Advance 1.5–4.5 mm. Stay low angle. Follow the yellow guide ring.' },
  { n: 3, name: 'Hydrodissection', inst: 'Hydro Cannula', tip: 'Insert 1–3 mm under the capsule edge and inject a fluid wave.' },
  { n: 4, name: 'Phacoemulsification', inst: 'Phaco Tip', tip: 'Advance 5–8 mm into the nucleus. Watch chamber stability.' },
  { n: 5, name: 'Cortex Removal', inst: 'I/A Handpiece', tip: 'Aspirate cortex. Keep vacuum < 280 mmHg, stability > 70%.' },
  { n: 6, name: 'IOL Insertion', inst: 'IOL Injector', tip: 'Advance 0.5–3.5 mm. Slow, controlled delivery into the bag.' },
  { n: 7, name: 'Wound Hydration', inst: 'Hydro Cannula', tip: 'Hydrate incision stroma to seal. Confirm watertight closure.' },
];
    ...
    <div className="text-[11px]">
      ...
      {tab === 'quickstart' && (
        <div className="space-y-2.5 text-blue-200/70 leading-relaxed">
          ...
          {[
            { n: '1', title: 'Select Procedure', body: 'Open Procedure section → choose Cataract Surgery.' },
            { n: '2', title: 'Place Entry Point', body: 'Press P → click anywhere on the white sclera to place your incision site.' },
            { n: '3', title: 'Start Curriculum', body: 'Open Surgery Curriculum → click Start Procedure. Follow the Step Guide at the top of the screen.' },
          ].map((s) => (
          ...
          <div className="mt-3 rounded-lg border border-amber-500/20 bg-amber-500/5 p-2 text-amber-200/70">
            ...
            <p>Once you place an entry point, the eye locks (🔒). Click the pill in the bottom-left to unlock camera orbit for inspection, then re-lock before operating.</p>
          </div>
          <div className="mt-2 rounded-lg border border-blue-500/20 bg-blue-500/5 p-2 text-blue-200/70">
            ...
            <p>Move the correct instrument to the depth shown in the Step Guide, then press ↻ Validate in the Curriculum section.</p>
          </div>
        </div>
      )}
      ...
      {tab === 'curriculum' && (
        <div className="space-y-2">
          {STEPS.map((s) => (
            <div
              key={s.n}
              className="rounded-lg border border-blue-500/15 bg-blue-500/5 p-2"
            >
