...
export interface ScoringAISlice {
  ...
  updateScoring: (pose: InstrumentPose | null, deltaTime: number, complicationsCount: number) => void;
  getCoachingFeedback: (currentStep: CataractStep, complicationsCount: number) => CoachingFeedback | null;
  resetScoringAndCoaching: () => void;
}

export const createScoringAISlice: StateCreator<SimulationState, [], [], ScoringAISlice> = (set) => ({
